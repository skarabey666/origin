...
<?include_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/wizards/aspro/stroy/css/styles.css");?>
<script>
	<?require_once($_SERVER['DOCUMENT_ROOT']."/bitrix/wizards/aspro/stroy/js/jquery-1.8.3.min.js");?>
	<?require_once($_SERVER['DOCUMENT_ROOT']."/bitrix/wizards/aspro/stroy/js/jquery.keyboard.js");?>
	function setWizardBackgroundColor(theme){
		window.console&&console.log(theme);
		switch (theme) {
			case "1":
				$(".instal-bg").css("backgroundColor", "#5baa00");
				break;
			case "2":
...
class SelectTemplateStep extends CSelectTemplateWizardStep{
	function InitStep(){
		...
		$wizard->SetDefaultVars(Array("templateID" => "stroy"));
	}
...
class SelectThemeStep extends CSelectThemeWizardStep{
	...
	function ShowStep(){
		...
		$arCurSiteTheme = CStroy::GetBackParametrsValues($siteID);
		...
		if (CModule::IncludeModule("aspro.stroy")){
			...
			$wizard->SetDefaultVars(
				Array(
					...
					"siteAddress" => strip_tags($this->GetFileContent(WIZARD_SITE_PATH."include/site-address.php", GetMessage("WIZ_COMPANY_ADDRESS_DEF"))),
...
class DataInstallStep extends CDataInstallWizardStep{
	...
	function CorrectServices(&$arServices){
		if($_SESSION["BX_stroy_LOCATION"] == "Y") $this->repeatCurrentService = true;
...
class FinishStep extends CFinishWizardStep{
	...
	function ShowStep(){
		...
		if(CModule::IncludeModule("aspro.stroy")) {
			CStroy::sendAsproBIAction("wizard_installed");
		}

		COption::SetOptionString("aspro.stroy", "WIZARD_DEMO_INSTALLED", "Y");
	}
}
?>