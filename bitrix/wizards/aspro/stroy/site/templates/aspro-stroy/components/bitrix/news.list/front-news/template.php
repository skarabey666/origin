...
<div class="news front">
	...
	<?if($arParams['PAGER_SHOW_ALL']):?>
		<a href="<?=str_replace('#SITE'.'_DIR#', SITE_DIR, $arResult['LIST_PAGE_URL'])?>" class="btn btn-default white"><span><?=GetMessage('S_TO_ALL_NEWS')?></span></a>
	<?endif;?>
				...
				<div class="item<?=($bImage ? '' : ' wti')?>" id="<?=$this->GetEditAreaId($arItem['ID'])?>">
					...
					<div class="info">
						...
						<?if(in_array('NAME', $arParams['FIELD_CODE']) && strlen($arItem['NAME'])):?>
							<div class="title">
								<?if($bDetailLink):?><a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="color_link"><?endif;?>
									<?=$arItem['NAME']?>
								<?if($bDetailLink):?></a><?endif;?>
							</div>
						<?endif;?>
