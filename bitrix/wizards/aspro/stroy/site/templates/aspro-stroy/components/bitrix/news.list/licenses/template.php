...
<div class="item-views box <?=$arParams['VIEW_TYPE']?> <?=($arParams['IMAGE_POSITION'] ? 'image_'.$arParams['IMAGE_POSITION'] : '')?> <?=($templateName = $component->{'__parent'}->{'__template'}->{'__name'})?>">
	<?// top pagination?>
							...
							<?if($bImage):?>
								<?ob_start();?>
									<div class="image">
										...
										<?elseif($arItem['FIELDS']['DETAIL_PICTURE']):?><a href="<?=$imageDetailSrc?>" alt="<?=($bImage ? $arItem['FIELDS']['PREVIEW_PICTURE']['ALT'] : $arItem['NAME'])?>" title="<?=($bImage ? $arItem['FIELDS']['PREVIEW_PICTURE']['TITLE'] : $arItem['NAME'])?>" class="img-inside fancy_gallery">
										<?endif;?>
										...
										<?if($arItem["GALLERY"]){?>
											<div class="popup_gallery" style="display:none;">
												<?foreach($arItem["GALLERY"] as $arGalery){?>
													<div class="img" data-title="<?=$arGalery['TITLE']; ?>" data-big="<?=$arGalery['DETAIL']['SRC']; ?>"></div>
												<?}?>
											</div>
										<?}?>
									</div>
