...
if(COption::GetOptionString("aspro.stroy", "wizard_installed", "N") == 'N'){
	...
	//fclose($file);
	COption::SetOptionString("aspro.stroy", "wizard_installed", "Y");
}
...
if($sitePhone2){
	$p2 = strpos($sitePhone2, ')');
	$sitePhone02 = substr($sitePhone2, 0, ($p2 + 1));
	$sitePhone03 = substr($sitePhone2, ($p2 + 1));
	$sitePhoneAll2 = preg_replace('@[^\d+]*@', '', $sitePhone2);
	$contact_phone.="<br/>".$sitePhone2;
	$phone2='<a href="tel:'.$sitePhoneAll2.'" rel="nofollow"><span>'.$sitePhone02.'</span>'.$sitePhone03.'</a>';
}
...
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."include/contacts-site-phone.php", Array("SITE_PHONE" => $contact_phone));
CWizardUtil::ReplaceMacros(WIZARD_SITE_PATH."include/site-phone.php", Array("SITE_TOP_2" => $phone2));
// email
...
// socials
$arCurSiteTheme = CStroy::GetBackParametrsValues(WIZARD_SITE_ID);
...
COption::SetOptionString('aspro.stroy', "OPTIONS", serialize($arCurSiteTheme), "", WIZARD_SITE_ID);
