...
$dbHblock = HL\HighloadBlockTable::getList(array("filter" => array("NAME" => "AsproStroyProfitReference")));
if(!$dbHblock->Fetch()){
	$data = array('NAME' => 'AsproStroyProfitReference', 'TABLE_NAME' => 'stroy_profit_reference');
	$result = HL\HighloadBlockTable::add($data);
	$HL_ID = $result->getId();
	$_SESSION["STROY_HBLOCK_COLOR_ID"] = $HL_ID;
	...
	if($HL_ID){
		...
		// list user options
		CUserOptions::SetOption("list", "tbl_".$data["TABLE_NAME"], array(
			'columns' => 'ID,UF_NAME,UF_SORT,UF_XML_ID,UF_LINK,UF_FILE', 'by' => 'sort', 'order' => 'asc', 'page_size' => '20',
		));
	}
}
?>